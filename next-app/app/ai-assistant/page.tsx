  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hello! I\'m your Gargash Motors AI Assistant. How can I help you find your perfect vehicle today?',
      timestamp: new Date()
    }
  ]);
                ...
                >
                  ...
                  {message.recommendations && message.recommendations.length > 0 && (
                    <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {message.recommendations.map((car, i) => (
                        <Link href={`/cars/${car._id}`} key={car._id}>
                          <div className="border rounded-md p-2 bg-white flex hover:bg-neutral-50 transition-colors">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-neutral-500">{car.year} · {car.bodyType}</p>
                              <p className="text-xs font-medium mt-1">${car.price.toLocaleString()}</p>
                            </div>
                          </div>
                        </Link>
                      ))}
                  ...
                  {message.action && (
                    <div className="mt-4">
                      <Link 
                        href={
                          message.action === 'SCHEDULE_TEST_DRIVE' ? '/test-drive' :
                          message.action === 'SHOW_CONTACT_INFO' ? '/contact' :
                          message.action === 'SHOW_FINANCING_OPTIONS' ? '/financing' :
                          '/'
                        }
                      >
                        <Button size="sm" className="w-full">
                          {message.action === 'SCHEDULE_TEST_DRIVE' ? 'Schedule Test Drive' :
                           message.action === 'SHOW_CONTACT_INFO' ? 'Contact Information' :
                           message.action === 'SHOW_FINANCING_OPTIONS' ? 'Financing Options' :
                           'Learn More'}
                        </Button>
                      </Link>
                    </div>
