...
async function processAiMessage(message: string, userPreferences: any) {
  ...
  if (lowercaseMsg.includes('recommend') || 
    ...
    if (userPreferences) {
      if (userPreferences.bodyTypes?.length > 0 && !query.bodyType) {
      ...
      if (userPreferences.fuelTypes?.length > 0 && !query.fuelType) {
    ...
    if (cars.length > 0) {
      ...
      return {
        ...
        recommendations: await Car.find().sort({ popularityScore: -1 }).limit(3)
      };
    }
  } 
  
  // Handle test drive scheduling queries
  else if (lowercaseMsg.includes('test drive') || lowercaseMsg.includes('appointment')) {
    return {
  ...
  else if (lowercaseMsg.includes('hours') || lowercaseMsg.includes('location') || lowercaseMsg.includes('address')) {
    return {
  ...
  else if (lowercaseMsg.includes('price') || lowercaseMsg.includes('cost') || lowercaseMsg.includes('financing')) {
    return {
